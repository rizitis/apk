...
def compare_package_versions(old_file, new_file, changelog_file='ChangeLog.txt'):
    ...
    def extract_version(pkg):
        """Extract the version components from a package string."""
        parts = pkg.split('-')
        return parts if len(parts) >= 3 else []  # Return empty list if not in expected format
    ...
    for new_package in new_packages:
        if new_package not in old_set:
            ...
            new_version = extract_version(new_package)

            if old_version:
                old_version = extract_version(old_version[0])
